...
import {
  ...
  Sun, Moon, LogOut, ChevronLeft, ChevronRight, Bell, Settings, User, Landmark, Trash2, X
} from "lucide-react";

import { getNotifications, markNotificationsRead, deleteNotifications } from "@/actions/notificationActions";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const loadNotifications = async () => {
    if (!userId) return;
    const data = await getNotifications(userId, 10); // Fetch top 10 for the bell dropdown
  ...
  const handleDismiss = async (e: any, notifId: string) => {
    e.stopPropagation();
  ...
  // Helper to format timestamps like "2h ago"
  ...
  if (!mounted || status === "loading" || status === "unauthenticated") {
    return <div className="flex h-[100dvh] w-full overflow-hidden bg-slate-50 dark:bg-[#0B0F19] transition-colors duration-300"></div>;
  }
                ...
                <div className="fixed top-20 left-4 right-4 md:absolute md:top-full md:left-auto md:-right-4 md:mt-2 md:w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-2xl md:shadow-xl rounded-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                  <div className="p-4 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
                    <h3 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                      Notifications {unreadCount > 0 && <span className="bg-[var(--color-brand-deep)] text-white text-[10px] px-2 py-0.5 rounded-full">{unreadCount}</span>}
                    </h3>
                    {unreadCount > 0 && (
                      <button 
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notif) => {
                        ...
                        return (
                          <div key={notif.id} className={`p-4 border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer group ${isUnread ? 'bg-slate-50/50 dark:bg-white/5' : ''}`}>
                            <div className="flex justify-between items-start mb-1">
                              <h4 className={`text-sm pr-4 ${isUnread ? 'font-bold text-slate-900 dark:text-white' : 'font-medium text-slate-700 dark:text-slate-300'}`}>
                                {notif.title}
                              </h4>
                  ...
                  <div className="p-3 border-t border-slate-200 dark:border-white/10 text-center bg-slate-50/50 dark:bg-white/5">
                    <Link href="/notifications" onClick={() => setShowNotifications(false)} className="text-xs font-bold text-[var(--color-brand-deep)] hover:text-[var(--color-brand-light)] transition-colors">
