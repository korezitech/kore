...
// Dummy data for notifications so we have something to show
const notifications = [
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // State for our new interactive menus
  ...
  useEffect(() => {
    ...
    // Logic to close dropdowns if you click anywhere else on the screen
        ...
        <header className="h-20 flex items-center justify-between px-6 md:px-10 border-b border-slate-200/60 dark:border-white/5 bg-white/40 dark:bg-black/20 backdrop-blur-xl absolute top-0 left-0 right-0 z-10">
          ...
          <div className="flex items-center gap-3 md:gap-5">
            {mounted && (
              <>
                ...
                <div className="relative" ref={notifRef}>
                  ...
                  {showNotifications && (
                    <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-xl rounded-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
                      <div className="p-4 border-b border-slate-200 dark:border-white/10 flex justify-between items-center">
                        <h3 className="font-semibold text-slate-900 dark:text-white">Notifications</h3>
                        <span className="text-xs text-[var(--color-brand-deep)] cursor-pointer hover:underline">Mark all read</span>
                      </div>
                        ...
                        {notifications.map((notif) => (
                          <div key={notif.id} className={`p-4 border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 cursor-pointer ${notif.unread ? 'bg-slate-50/50 dark:bg-white/5' : ''}`}>
                            <div className="flex justify-between items-start mb-1">
                              <h4 className={`text-sm ${notif.unread ? 'font-semibold text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>{notif.title}</h4>
            ...
            <div className="relative" ref={profileRef}>
              ...
              >
                <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-[var(--color-brand-deep)] to-[var(--color-brand-light)] p-[2px] shadow-sm group-hover:scale-105 transition-transform">
                  <div className="w-full h-full rounded-full bg-white dark:bg-slate-900 flex items-center justify-center">
                    <span className="font-bold text-xs text-[var(--color-brand-deep)]">ME</span>
                  </div>
                </div>
              </button>
              ...
              {/* Profile Dropdown */}
              {showProfileMenu && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-xl rounded-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 p-2">
                  <div className="px-3 py-2 border-b border-slate-100 dark:border-white/5 mb-2">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 truncate">admin@korefinance.com</p>
                  </div>
                  
                  <Link href="/dashboard/profile" className="flex items-center gap-2 px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors">
                    <User className="w-4 h-4" /> Profile Settings
                  </Link>
                  <Link href="/admin" className="flex items-center gap-2 px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors">
                    <Settings className="w-4 h-4" /> Admin Panel
                  </Link>
                  
                  <div className="h-px bg-slate-100 dark:bg-white/5 my-2"></div>
                  
                  <button className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors w-full text-left">
